...
export function DashboardConsole() {
  ...
  const [loadedSuggestionsKey, setLoadedSuggestionsKey] = React.useState("")

  // Suggestions are generated by an LLM call on the backend, so they are
  // only fetched when the user actually opens the suggestions picker —
  // never on mount/refresh. Deduped per (org, dataset, agent): reopening
  // the picker for the same selection reuses the cached result.
  const suggestionsAbortRef = React.useRef<AbortController | null>(null)
  const selectionKey =
    organization && datasetId && activeAgent
      ? `${organization.id}|${datasetId}|${activeAgent.id}`
      : ""
  // Stale results (from a previous selection) are hidden via derivation
  // rather than an effect, so the button falls back to "Suggestions" until
  // the picker is reopened for the current selection.
  const effectiveSuggestions =
    loadedSuggestionsKey === selectionKey ? suggestions : []

  const loadSuggestions = React.useCallback(() => {
    if (!organization || !datasetId || !activeAgent) return

    const key = `${organization.id}|${datasetId}|${activeAgent.id}`
    if (key === loadedSuggestionsKey) return
  ...
  }, [organization, datasetId, activeAgent, loadedSuggestionsKey])

  // Abort any in-flight request if the console unmounts mid-load.
  React.useEffect(() => {
    return () => {
      suggestionsAbortRef.current?.abort()
    }
    ...
    <div className="flex h-full flex-col gap-2">
      ...
      <div className="pb-4">
        <QueryBar
          ...
          onOpenSuggestions={loadSuggestions}
